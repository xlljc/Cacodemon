...
if(ckJ){
	...
	if(st == plSt.jumpFall && ckS){
		...
		if(!atkCoolingTimer4){
			//箭袋箭剩余量
			var tempNum = weaponIndex ? weaponBox2Var[? "arrow"] : weaponBox1Var[? "arrow"];
			//近战范围内是否有敌人
			var temp = collision_rectangle(x + (image_xscale ? 30 : -30),y,x - 5,y - 40,ob_biology_monster_f,0,0);
			if(tempNum != undefined && tempNum > 0 && (!temp || temp.st == monsterSt.dead)){
				animationName = spAtk;
				atkCoolingFrame4 = 30;
			}else {
				animationName = "atkNoneArrow";
				atkCoolingFrame4 = 42;
			}
			skeleton_animation_set_ext(animationName,0);
			animationIndex = 0;
