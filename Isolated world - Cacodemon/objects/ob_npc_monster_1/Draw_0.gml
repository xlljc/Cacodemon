...
#region 变量区
	...
	//判断下一步前方是否有墙体遮挡
	var nextHasWall = position_meeting(x + (image_xscale ? 50 : -50),y - 30,ob_floor_collision);
	//运动变量
	po = 0;
	//判断玩家是否在视野里
...
#region 事件执行
	...
	if(event == "atk"){
		if(!atkTimer && image_index >= 2 && image_index < 2 + imageSpeed){
			var list = ds_list_create();
			var num = collision_rectangle_list(x + (image_xscale ? 40 : -40),y,x,y - 40,ob_biology_teammate_f,0,0,list,0);
			for(var i = 0;i < num;i ++){
				sc_teammate_lostHp(list[| i],image_xscale ? 10 : -10,0);
			}
			ds_list_destroy(list);
			//物体交互
			sc_object_touch_rectangle(x + (image_xscale ? 40 : -40),y,x,y - 40,undefined,0);
			atkTimer = atkCoolingFrame1;
		}else if(image_index >= sprite_get_number(spAtk) - imageSpeed){ //动画结束，状态改变
			st = monsterSt.idle;
		}
	}
#endregion
...
#region debug
	//draw_rectangle(x + (image_xscale ? viewForwardLen : -viewForwardLen),y - 10,x,y - 40,1);
	//deBugStr += "hp = " + string(presentHp);
	//deBugStr += "\nsprite = " + sprite_get_name(sprite_index);
	//deBugStr += "view = " + string(sc_instance_in_view(hasPlayer,self,hasWall));
	//deBugStr += "\nwall = " + string(hasWall);
	//deBugStr += "\nst = " + sc_get_enumName(st);
	//if(hasPlayer){
	//	draw_line(x,y,hasPlayer.x,hasPlayer.y);
	//}
	//deBugStr += "st= " + string(sc_get_enumName(st));
